...
```{r instalcscldcsceBSded,cache=TRUE,eval=FALSE}
library(soGGi)
library(ggplot2)
library(TxDb.Mmusculus.UCSC.mm10.knownGene)
...
```{r instalcscldcsceBSdead,cache=TRUE,eval=TRUE}
...
```{r instaaxscslldebbzBSccdxxed,cache=TRUE,dependson="instalcscldcsceBSded"}
...
```{r instaaxscslldeabbBSccded, eval=TRUE,dependson="instalcscldcsceBSded"}
sortedBAM <- "~/Downloads/ENCFF053CGD.bam"
...
save(CTCF_Cuts_open,file="../Data/cuts_Treg.RData")
...
library(rGREAT)
great_Job <- submitGreatJob(LiverMinusHindbrain, species = "mm10",request_interval = 4)
